...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, appListOnInput: appsList}
  ...
  onChangeInput = event => {
    const {appListOnInput} = this.state
    const userInput = event.target.value
    const filteredListOnInput = appListOnInput.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(userInput.toLowerCase()),
    )
    this.setState({appListOnInput: filteredListOnInput})
  }

  render() {
    const {activeTabId} = this.state
    const {appListOnInput} = this.state
    const filteredOnCategory = appListOnInput.filter(
        ...
        <ul className="tab-items-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              activeBtnEffect={
                eachTab.tabId === activeTabId ? 'active-btn' : ''
              }
            />
          ))}
        </ul>
        <ul className="app-items-container">
          {filteredOnCategory.map(eachItem => (
            <AppItem eachItemDetails={eachItem} key={eachItem.id} />
          ))}
        </ul>
      </div>
    )
